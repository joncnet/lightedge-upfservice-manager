/**
 * Support global variable for storing the "alerts in LIGHTEDGE and referring them 
 * and their property univocally in the WEBUI
 */

__LIGHTEDGE_WEBUI.ALERT={
 ...
 * 
 * @requires js/support/lightedge-global.js 
 * @requires js/support/lightedge-corefunctions.js
 * 
...
class WEBUI_Alert extends WEBUI_CoreFunctions{
   ...
   * @param {string} type - the type of alert. Refer __LIGHTEDGE_WEBUI.ALERT.TYPES  
   *                        for valid values
   * @param {string} strong_text  - the main text of the alert.
   * @param {string} text -the secondary text of the alert.
   */
  constructor(alert_id, type=__LIGHTEDGE_WEBUI.ALERT.TYPES.ERROR, strong_text="", 
              text=""){
   ...
   */
  get_$main(alert_id=null, type=__LIGHTEDGE_WEBUI.ALERT.TYPES.ERROR){
    let alert_type = "alert-error"
    switch(type){
      case __LIGHTEDGE_WEBUI.ALERT.TYPES.SUCCESS:
        alert_type = "alert-success"
        break
      case __LIGHTEDGE_WEBUI.ALERT.TYPES.WARNING:
  ...
  publish(top=true, destroy_overlapping=true){
    ...
    if (top){
      $("#"+__LIGHTEDGE_WEBUI.ALERT.ALERT_BOX_ID).prepend(this.get_$instance())
    }
    else{
      $("#"+__LIGHTEDGE_WEBUI.ALERT.ALERT_BOX_ID).append(this.get_$instance())
    }
 ...
 */
function lightedge_alert_format_content(args){
  let jQxhr = null
  let detail = args[1]
  ...
  args.forEach(function(arg, index){
    ...
    if ((arg != null) && (arg != undefined)){
      if ((arg.lightedge != null) && (arg.lightedge != undefined)){
        jQxhr = arg
      }
    }
  })
  ...
  if (jQxhr != null){
    text = text + "<em class='text-xs'>" +
                  jQxhr.lightedge.method + " : " + jQxhr.lightedge.url +
                  " [@" + jQxhr.lightedge.time.substring(11) + "]</em>"
  }
 ...
 * @param  {...any} args - arguments returned by the HTTP response
 */
function lightedge_alert_generate_success(...args) {

  let alert_content= lightedge_alert_format_content(args)
 ...
 * @param  {...any} args - arguments returned by the HTTP response
 */
function lightedge_alert_generate_error(...args) {

  let alert_content= lightedge_alert_format_content(args)

  new WEBUI_Alert_Error("alert_" + lightedge_alert_assign_id_number(),
                        alert_content.strong_text,
                        alert_content.text)
    .generate().show()
}
 ...
 */
function lightedge_alert_assign_id_number(){
  __LIGHTEDGE_WEBUI.ALERT.COUNTER = 
    (__LIGHTEDGE_WEBUI.ALERT.COUNTER + 1) % __LIGHTEDGE_WEBUI.ALERT.MAX_NUMBER
  
  return __LIGHTEDGE_WEBUI.ALERT.COUNTER
}
